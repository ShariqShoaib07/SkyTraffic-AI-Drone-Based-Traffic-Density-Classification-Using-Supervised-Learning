...
## Overview
This project builds an end-to-end pipeline for vehicle detection and traffic density classification from street-level images. It trains or loads a YOLO detector, counts vehicles per image, and assigns each scene a Low, Medium, or High traffic label. Those labels and detections feed a classical ML classifier that predicts traffic density from engineered features.

The raw dataset lives under [dataset/dataset](dataset/dataset) and is organized into four street-level sections (sec1-sec4) with YOLO-format label files that store class_id cx cy width height values in normalized coordinates. The dataset is roughly 150k images, with each image paired to a TXT label file containing bounding boxes and class IDs.
...
## Project Structure
- [inspect_dataset.py](inspect_dataset.py) - Scans sec1-sec4 for class ID counts and prints sample image names.
...
- `RandomForestClassifier` - Supervised classifier for traffic density using engineered features.
- `SVC` (RBF kernel) - Support Vector Machine classifier for traffic density.
- `MLPClassifier` - Multi-layer perceptron classifier for traffic density.
...
## Results
The training phase produces per-model confusion matrices, feature-importance visualizations (Random Forest), a model comparison chart, and training summaries that report best accuracy, cross-validation scores, and top features. The statistical analysis phase expands this with dataset distribution plots, feature correlation and distribution analyses, ROC curves, a performance radar chart, and a consolidated statistical report with accuracy, precision, recall, and F1-score metrics for each classifier.
